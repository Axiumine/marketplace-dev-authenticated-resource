...
# Redis cluster, the REAL MongoDB and a REAL clamd (REDIS_* / MONGODB_URI from .env, clamd on
# /var/run/clamav/clamd.ctl — start() arms the antivirus before the server is built). If any
# of the three is unreachable, integration bails and lcov.info keeps the PREVIOUS run's
# numbers — Qodana would then scan stale coverage. Ensure all three are reachable before
# trusting the coverage verdict. See COVERAGE.md.
