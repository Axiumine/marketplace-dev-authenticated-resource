...
export function onUnhandledRejection(reason: unknown): void {
	...
	// Synchronous handler: `await` is not available here, so the exit is chained onto the flush
	// instead of ordered after it. Either way the event has left the process before it dies.
...
export async function start() {
	...
	try {
		...
		// Flushed inside disconnectAllDatabases, immediately ahead of its own process.exit() — this
		// event and whatever it reports join the same queue and leave before the process does.
		Sentry.captureException(error)
