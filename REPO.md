...
`prettier --check`, both over the whole tree), then `yarn typecheck` (`tsconfig.test.json` over src/,
test/ and the vitest configs, no emit), then `yarn test:cov` (100% on every metric), then
