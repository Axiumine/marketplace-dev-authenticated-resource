...
[![OpenSSF Scorecard](https://api.scorecard.dev/projects/github.com/Axiumine/marketplace-dev-authenticated-resource/badge)](https://scorecard.dev/viewer/?uri=github.com/Axiumine/marketplace-dev-authenticated-resource)
...
a form loaded before an admin unpublished something would put it straight back without asking to.
`companyAdd` and `itemAdd` stamp `false`; the two `*UpdatePublished` mutations are the only writers of
the flag on this tier, matching the admin tier's `itemUpdatePublished` on 4024.
